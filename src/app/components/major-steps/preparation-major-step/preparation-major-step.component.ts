...
export class PreparationMajorStepComponent implements AfterViewInit {
    ...
    onFormValueChange(essayTemplateStep: EssayTemplateStep): void {
        this.runEssayService.getEssayStep(essayTemplateStep.id)?.patchValue(essayTemplateStep as EssayStep);
    }

    markVerifiedStep(essayStep: EssayStep): void {
        this.runEssayService.getEssayStep(essayStep.id).get('verifiedStatus')?.setValue(StepStatus.Done);
    ...
    assignMetersToStands$(): Observable<EssayStep> {
        ...
        return this.dbServiceMeter
            ...
            .pipe(
                first(),
                map(({ rows, relations }) =>
                    RelationsManager.mergeRelationsIntoRows<Meter>(rows, relations, MeterDbTableContext.foreignTables)
                ),
                map((meters) => {
                    if (!this.preparationStep) {
                        throw new Error('Paso de preparación no encontrado');
                    }
                    const preparationEssayStepForm = this.runEssayService.getEssayStep(this.preparationStep.id);
                    (
                        preparationEssayStepForm.get('form_control_raw') as FormArray<AbstractFormGroup<Stand>>
                    ).controls.forEach((standControl) => {
                        const meter = meters.find(({ id }) => standControl.getRawValue().meter_id === id);
