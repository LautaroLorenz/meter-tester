...
export class VerificationMajorStepComponent implements OnInit {
    ...
    markVerifiedAll(): void {
        ...
        this.verificationSteps?.forEach((essayStep) => this.markVerifiedStep(essayStep, StepStatus.Done));
    }

    markVerifiedStep(essayStep: EssayStep, status: StepStatus): void {
        this.runEssayService.getEssayStep(essayStep.id).get('verifiedStatus')?.setValue(status);
