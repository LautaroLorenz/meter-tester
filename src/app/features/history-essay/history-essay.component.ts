...
import { HistoryEssay, HistoryEssayDbTableContext } from '../../models/business/database/history-essay.model';
...
export class HistoryEssayComponent implements OnInit, OnDestroy {
    ...
    deleteHistoryEssay(): void {
        ...
        this.confirmationService.confirm({
            ...
            accept: () => {
                this.dbService
                    .deleteTableElements$(HistoryEssayDbTableContext.tableName, [this.historyEssay?.id])
                    .pipe(
                        first(),
                        tap(() => {
                    ...
                    .subscribe({
                        error: () => this.messagesService.error('No se pudo eliminar los registros')
                    });
            }
        });
    }
