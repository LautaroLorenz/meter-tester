...
import { Observable, takeUntil, tap, startWith, Subject, take, map, concat, catchError, throwError } from 'rxjs';
...
export class StaticsComponent implements OnInit, OnDestroy {
    ...
    private reloadWidgets(before: Date, after: Date): void {
        ...
        observables.push(
            this.staticsService.getMetric$(Metric.meterModelApproved, before, after).pipe(
                take(1),
                map((metrics) => metrics.map(({ tags_raw }) => tags_raw)),
        ...
        observables.push(
            this.staticsService.getMetric$(Metric.meterModelFailed, before, after).pipe(
                take(1),
                map((metrics) => metrics.map(({ tags_raw }) => tags_raw)),
