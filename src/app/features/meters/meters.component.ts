...
import { Connection, ConnectionDbTableContext } from '../../models/business/database/connection.model';
...
export class MetersComponent extends AbmPage<Meter> implements OnDestroy {
    ...
    readonly cols: TableColumn<Meter>[] = [
        ...
        {
            field: (meter) => `${meter.activeConstantValue} [${meter.foreign.activeConstantUnit.name}]`,
        ...
        {
            field: (meter) => `${meter.reactiveConstantValue} [${meter.foreign.reactiveConstantUnit.name}]`,
    ...
    constructor(
        ...
        this.form = this.fb.group({
            ...
            activeConstantUnit_id: this.fb.control(undefined, Validators.required.bind(this)),
    ...
    private readonly updateDropdownOptions = (): void => {
        ...
        if (this._relations[BrandDbTableContext.tableName]) {
            this.dropdownBrandOptions = this._relations[BrandDbTableContext.tableName].sort((a, b) =>
                a.name.localeCompare(b.name)
            );
        }
        if (this._relations[ConnectionDbTableContext.tableName]) {
            this.dropdownConnectionOptions = this._relations[ConnectionDbTableContext.tableName].sort((a, b) =>
                a.name.localeCompare(b.name)
            );
        }
    };
