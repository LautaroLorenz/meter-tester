...
export class RunEssayComponent implements OnInit, OnDestroy {
    ...
    private observeRoute(): void {
        this.id$
            .pipe(
                ...
                switchMap((id) =>
                    this.dbServiceEssayTemplate.getTableElement$(EssayTemplateDbTableContext.tableName, id)
                ),
                tap(({ id, name }) =>
                    this.runEssayForm.patchValue({
                        essayTemplateId: id,
                        essayName: name,
                        startDate: this.formatDate.transform(new Date(), FormatDateMode.fromClientToDatabase) as string
                    })
                ),
    ...
    private observeTables(): void {
        this.dbServiceEssayTemplateStep
            ...
            .pipe(
                ...
                tap((essayTemplateSteps) => this.runEssayService.buildSteps(essayTemplateSteps)),
                tap(() => {
    ...
    private requestTableEssayTemplateSteps(essayTemplateId: number): void {
        ...
        const foreignTablesFiltered = foreignTables.filter(({ tableName }) => tableName !== essayTemplateTableName);
