...
export class ExecutionDirector {
    static getInitialStandResultStatus(preparatonStep: PreparationStep, standIndex: number): ResultStatus {
        if (!preparatonStep.form_control_raw[standIndex]) {
            return ResultStatus.NotApply;
        }

        return preparatonStep.form_control_raw[standIndex].isActive ? ResultStatus.Pending : ResultStatus.NotApply;
    }
    ...
    private static getLastAdjustmentMeterConstant(essaySteps: EssayStep[], limitIndex: number): number | undefined {
