...
import { BehaviorSubject, Observable, Subject, exhaustMap, filter, interval, map, takeUntil, takeWhile } from 'rxjs';
            ...
            filter(({ result, error }) => {
                if (error) {
                    ...
                    if (onError) {
                        ...
                        this.messagesService.error(`Error de comunicación [${DeviceConstants[this.device]}]`);
                    }
                    return false;
                }
                return !!result;
