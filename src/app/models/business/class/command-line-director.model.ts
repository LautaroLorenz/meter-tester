...
import { CommandBlock, CommandBlockTypes } from '../interafces/command-block.model';
...
export class CommandLineDirector {
    static findCommandLine(
        ...
        const possibleCommandLineToResponse = commandLines.filter(({ name }) => name === responseCommandName);
        ...
        for (let i = 0; i < history.length; i++) {
            ...
            const commandLine = possibleCommandLineToResponse.find(({ enableConditions }) => {
    ...
    static refreshBlockValue(block: CommandBlock): CommandBlock {
        ...
        if (block.variableValue !== null && block.config.probabilityOfChange < Random.range(0, 100)) {
            return block;
        }
        ...
        switch (block.config.type) {
            case CommandLineConfigTypes.Incremental:
                variableValue = this.getBlockIncrementalValue(block.variableValue, block.config);
                break;
    ...
    static formatVariableValueToValue(
        ...
        const value: string = variableValue.toString().padStart(digitsQuantity, padText);
        const end: string = endWith ?? '';
        return `${start}${value}${end}`;
    }

    static getBlockIncrementalValue(value: null | string | number, config: CommandBlockConfigIncremental): number {
        if (value === null) {
            return 0;
        }
