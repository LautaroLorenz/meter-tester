...
export class MajorStepsDirector {
    static stepsByMajorStep(essaySteps: EssayStep[], majorStep: MajorSteps): EssayStep[] {
        switch (majorStep) {
            case MajorSteps.Verification:
                return essaySteps.filter(
                    (essayStep) => 'verifiedStatus' in essayStep && essayStep.step_id !== Steps.Preparation
                );
    ...
    static getMajorStepStatusMap(essaySteps: EssayStep[]): Record<MajorSteps, StepStatus> {
        ...
        const verificationSteps = this.stepsByMajorStep(essaySteps, MajorSteps.Verification);

        const preparationSteps = this.stepsByMajorStep(essaySteps, MajorSteps.Preparation);

        const executionSteps = this.stepsByMajorStep(essaySteps, MajorSteps.Execution);
        ...
        const isExecutedDone: boolean = this.checkMajorStepStatus(executionSteps, StepStatus.Done, 'executedStatus');

        return {
            [MajorSteps.Verification]: this.getMajorStepStatus(true, isVerificationDone),
            [MajorSteps.Preparation]: this.getMajorStepStatus(isVerificationDone, isPreparationDone),
            [MajorSteps.Execution]: this.getMajorStepStatus(isPreparationDone, isExecutedDone),
            [MajorSteps.Report]: this.getMajorStepStatus(isExecutedDone, false)
        };
    }

    private static getMajorStepStatus(isPreviousStepDone: boolean, isDone: boolean): StepStatus {
